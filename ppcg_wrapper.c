...
/* 
 * Create a new ppcg_options structure with default values. 
 */
struct ppcg_options *ppcg_options_new_with_defaults()
{
    struct ppcg_options *options;
    ...
    options = isl_calloc_type(isl_ctx_alloc(), struct ppcg_options);
    if (!options)
        return NULL;
    
    options->debug = (struct ppcg_debug_options *)malloc(sizeof(struct ppcg_debug_options));
    if (!options->debug) {
        free(options);
        return NULL;
    }
    
    options->debug->dump_schedule_constraints = 0;
    options->debug->dump_schedule = 0;
    options->debug->dump_final_schedule = 0;
    options->debug->dump_sizes = 0;
    options->debug->verbose = 0;
    
    options->group_chains = 1;
    options->reschedule = 1;
    options->scale_tile_loops = 1;
    options->wrap = 1;
    
    options->non_negative_parameters = 0;
    options->ctx = NULL;
    options->sizes = NULL;
    ...
    options->isolate_full_tiles = 0;
    
    options->use_private_memory = 1;
    options->use_shared_memory = 1;
    options->max_shared_memory = 8192;
    
    options->target = PPCG_TARGET_C;
    
    options->openmp = 0;
    options->linearize_device_arrays = 1;
    options->allow_gnu_extensions = 1;
    options->live_range_reordering = 1;
    
    options->hybrid = 0;
    options->unroll_copy_shared = 0;
    options->unroll_gpu_tile = 0;
    
    options->opencl_compiler_options = NULL;
    options->opencl_use_gpu = 1;
    options->opencl_n_include_file = 0;
    options->opencl_include_files = NULL;
    options->opencl_print_kernel_types = 1;
    options->opencl_embed_kernel_code = 0;
    
    options->save_schedule_file = NULL;
    options->load_schedule_file = NULL;
    
    options->isl = isl_options_new_with_defaults();
    
    return options;
}

/* 
 * Free the ppcg_options structure.
 */
void ppcg_options_free(struct ppcg_options *options)
{
    if (!options)
        return;
    ...
    free(options->ctx);
    free(options->sizes);
    free(options->opencl_compiler_options);
    free(options->save_schedule_file);
    free(options->load_schedule_file);

    isl_options_free(options->isl);
