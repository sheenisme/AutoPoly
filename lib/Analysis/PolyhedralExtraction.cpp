...
isl_ctx* PolyhedralUtils::createContext() {
  ...
  // Set options for better scheduling
...
bool PolyhedralUtils::isSuitableForPolyhedral(Operation* op) {
  // Check if operation is part of affine dialect
  if (isa<AffineForOp, AffineIfOp, AffineLoadOp, AffineStoreOp, AffineYieldOp>(op)) {
    return true;
  }
  
  // Check for arithmetic operations that are polyhedral-friendly
...
std::vector<AffineForOp> PolyhedralUtils::getAffineLoopNest(Operation* op) {
  std::vector<AffineForOp> nest;
  
  Operation* current = op;
  while (current) {
    if (auto forOp = dyn_cast<AffineForOp>(current)) {
      nest.insert(nest.begin(), forOp);
    }
...
std::vector<Value> PolyhedralUtils::getMemoryAccesses(Operation* op) {
  std::vector<Value> accesses;
  
  if (auto loadOp = dyn_cast<AffineLoadOp>(op)) {
    accesses.push_back(loadOp.getMemRef());
  } else if (auto storeOp = dyn_cast<AffineStoreOp>(op)) {
    accesses.push_back(storeOp.getMemRef());
  }
...
int PolyhedralUtils::getMaxLoopDepth(Region& region) {
  int maxDepth = 0;
